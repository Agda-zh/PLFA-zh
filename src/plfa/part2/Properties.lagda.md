...
* 如果这个项是一个 λ-抽象，那么项本身的良类型性保证了函数体也是良类型的。

* 如果这个项是零，那它自然是标准式。

* 如果这个项是一个后继表达式，项本身的良类型保证了它的参数也是良类型的；
  由于它是一个值，所以它的参数也是一个值。
  从而由归纳可证它的参数也是标准式。
...
变量的情况不予考虑，这是因为在闭项中不会出现自由变量，且变量不是值。
函数应用、分支表达式和不动点的情况也不予考虑，因为它们都不是值。
...
反过来说，如果一个项是标准式，那么它是一个在空上下文中良类型的值。
